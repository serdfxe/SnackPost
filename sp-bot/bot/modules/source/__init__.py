...
async def add_user_source(user_id: int, url: str, source_type: str) -> bool:
    ...
    try:
        res = await create_source_route_sources_post.asyncio(
            ...
            x_user_id=user_id,
            body=SourceCreateDTO(url=url, type_=SourceType[source_type.upper()]),
        )
...
async def get_user_sources(user_id: int) -> list[dict]:
    ...
    return await get_sources_route_sources_get.asyncio(
        client=scraper_client, x_user_id=user_id
    )
...
async def show_sources_menu(message: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="🗂 Мои источники", callback_data="list_sources"
                )
            ],
...
@source_router.message(
    SourcesStates.waiting_for_source_url, F.text & ~F.text.startswith("/")
)
            ...
            await callback.message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="🔙 Назад", callback_data="sources_menu"
                            )
                        ]
                    ]
                ),
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="🗑 Удалить", callback_data="delete_source"
                        )
                    ],
                    [
                        InlineKeyboardButton(
...
async def start_delete_source(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=f"❌ {source.url}",
                        callback_data=f"confirm_delete_{source.id}",
                    )
                ]
        ...
        await callback.message.edit_text(
            DELETE_SOURCE_MSG, parse_mode="HTML", reply_markup=keyboard
        )
        await callback.answer()
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="🔙 К списку", callback_data="list_sources"
                        )
                    ]
                ]
            ),
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="Утро (09:00)", callback_data="set_time_09:00"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text="День (14:00)", callback_data="set_time_14:00"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text="Вечер (19:00)", callback_data="set_time_19:00"
                        )
                    ],
                    [
                        InlineKeyboardButton(
