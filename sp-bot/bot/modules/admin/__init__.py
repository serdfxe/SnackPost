...
async def check_user_subscription(message: Message, user_id: int, bot: Bot):
    ...
    try:
        ...
        if sub_response:
            ...
            expires_at = subscription.expires_at.strftime("%d.%m.%Y %H:%M") if subscription.expires_at else "не указано"
...
async def grant_user_subscription(message: Message, user_id: int, duration_days: int, bot: Bot):
